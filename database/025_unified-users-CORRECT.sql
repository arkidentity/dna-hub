...
-- ============================================================================
-- NOTE: church_id can be NULL for roles that aren't church-specific (like training_participant)

CREATE TABLE IF NOT EXISTS user_roles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE NOT NULL,
  role TEXT NOT NULL CHECK (role IN ('church_leader', 'dna_leader', 'training_participant', 'admin')),
  church_id UUID REFERENCES churches(id) ON DELETE CASCADE, -- nullable
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(user_id, role, COALESCE(church_id, '00000000-0000-0000-0000-000000000000'::uuid))
);
